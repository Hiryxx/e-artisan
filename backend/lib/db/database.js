...
class Database {
    ...
    createUsers = `CREATE TABLE IF NOT EXISTS users (user_id INT PRIMARY KEY,password VARCHAR(255) NOT NULL,email VARCHAR(100) UNIQUE NOT NULL,name VARCHAR(50) NOT NULL,lastname VARCHAR(50) NOT NULL,cf VARCHAR(16) UNIQUE NOT NULL,role_id INT NOT NULL,info_id INT,FOREIGN KEY (role_id) REFERENCES user_roles(role_id),FOREIGN KEY (info_id) REFERENCES shipment_infos(info_id));`
    ...
    createProducts = `CREATE TABLE IF NOT EXISTS products (product_id INT PRIMARY KEY,price NUMERIC(10, 2) NOT NULL,id_category INT NOT NULL,description TEXT,seller_id INT NOT NULL,image_url TEXT,stock INT NOT NULL CHECK (stock >= 0),FOREIGN KEY (id_category) REFERENCES categories(id_category),FOREIGN KEY (seller_id) REFERENCES users(user_id));`
